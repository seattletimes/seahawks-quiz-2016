...
module.exports = function(grunt) {
  ...
  grunt.registerTask("build", "Processes index.html using shared data (if available)", function() {
    ...
    var finish = grunt.file.read("src/_finish.html");
    grunt.data.json.SeahawksPersonalityQuizV2_Players.forEach(function(person) {
      person.name = person.player.split(" ").pop().toLowerCase();
      var output = grunt.template.process(finish, { data: person });
      grunt.file.write("build/" + person.name + ".html", output);
    });
